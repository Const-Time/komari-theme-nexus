...
2. [specs/docs/NEXUS_SPEC.md](specs/docs/NEXUS_SPEC.md) - Nexus product and layout contract.
3. [AICACHE.md](AICACHE.md) - current AI work cache: planned work, completed work, validation, blockers, and handoff notes.
4. Nearest `AGENTS.md` for the files you are editing:
   - [AGENTS.md](AGENTS.md) - root/release/repo rules.
   - [src/AGENTS.md](src/AGENTS.md) - source-tree implementation rules.
5. Relevant docs under [docs/](docs/): architecture, auth, cache, data flow, migration, milestones.

## What this project is

Komari Nexus is a Komari Monitor theme built with Vue 3 + Vite. The release artifact is a Komari-importable zip package, not a generic deployed web app.
...
- Build output must include `dist/` and `komari-theme-nexus-build-<short-sha>.zip`.
...
- Nexus service state stays in [src/stores/nexus.ts](src/stores/nexus.ts); visible probe history stays in [src/stores/nexusHistory.ts](src/stores/nexusHistory.ts).
...
- Keep public home/detail routes public; do not add broad router guards. `/nexus-settings` redirects guests to Komari admin login.
- Gate sensitive actions/data paths through verified auth (`appStore.requireLoginPermission()` / auth service).
- Do not parse raw `theme_settings` in components; normalize in [src/stores/app.ts](src/stores/app.ts) or a dedicated store.
