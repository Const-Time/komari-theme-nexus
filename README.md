...
图标可以使用 Iconify 名称或 HTTP(S) 图片地址。设置页可自动获取 favicon，或上传 SVG、PNG、JPG、WebP、GIF、ICO；上传图片会在浏览器内清理并压缩后保存。
